...
module.exports = function startOperations(ctx, done)
{
  ...
  fs.writeFileSync(__dirname + '/../last-input.json', JSON.stringify({
    config: ctx.config,
    program: ctx.program,
    test: ctx.test
  }, null, 2));
  ...
  step(
    ...
    function connectToDeviceStep()
    {
      ...
      tryToConnect(1, this.next());

      function tryToConnect(tryCount, next)
      {
        ctx.client.open(function(err)
        {
          if (!err)
          {
            return next();
          ...
          if (tryCount < ctx.config.btConnectTryCount)
          {
            return tryToConnect(tryCount + 1, next);
          }
  ...
  function createBtTransport(connection)
  {
    var transport = new led4uc2.Transport(connection, {
      ...
      sendOrderDelay: ctx.config.btRequestDelay,
      maxRetryCount: ctx.config.btRequestRetryCount
    });
    ...
    {
      transport.on('request', function(req, tryNo) { util.log('[led4uc2#request#%d] %s', tryNo, req); });
